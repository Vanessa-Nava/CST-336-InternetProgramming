...
function getCategoriesHTML() {
   ...
   // $sql = "SELECT * FROM item WHERE name LIKE '%$query%'"; 
    ...
    $imgSQL = $showImages ? ', item.image_url' : ''; 
     ...
     $sql = "SELECT distinct item.item_id, item.name, item.price, item.image_url, category.category_name FROM item INNER JOIN item_category ON item.item_id = item_category.item_id INNER JOIN category ON item_category.category_id =category.category_id  WHERE 1"; 
